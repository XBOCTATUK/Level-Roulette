...
bool RouletteLevelListCell::init(std::string listName, matjson::Value levelData) {
    ...
    auto diff = levelData["diff"].asString().ok().value();
    ...
    auto levelName = levelData["name"].asString().ok().value();
    ...
    auto creatorName = levelData["creator"].asString().ok().value();
    ...
    auto deleteBtn = CCMenuItemExt::createSpriteExtra(deleteBtnSpr, [this, listName, levelData](auto) {
        std::string levelID = std::to_string(levelData["levelID"].asInt().ok().value());
